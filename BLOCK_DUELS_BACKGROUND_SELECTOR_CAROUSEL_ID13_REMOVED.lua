...
;(function()
    ...
    -- ============================================================
-- ============================================================
-- BUTTONBACKGROUNDIMAGESELECTOR (HORIZONTAL CAROUSEL STYLE)
-- ============================================================
do
    local BUTTON_BG_IMAGES = {
        ...
        "rbxassetid://96005510156373",
        "rbxassetid://90384569541777",
    }

    -- Image #13 was removed from this selector.
    do
        local savedIndex = tonumber(_configState["ButtonBackgroundIndex"])
        if savedIndex then
            if savedIndex == 13 then
                savedIndex = 1
            elseif savedIndex > 13 then
                savedIndex = savedIndex - 1
            end
            _configState["ButtonBackgroundIndex"] = savedIndex
        end
    end

    local currentButtonBgIndex = tonumber(_configState["ButtonBackgroundIndex"]) or 1
    if currentButtonBgIndex < 1 or currentButtonBgIndex > #BUTTON_BG_IMAGES then
        currentButtonBgIndex = 1
    end

    local function applySelection(index)
        currentButtonBgIndex = math.clamp(index, 1, #BUTTON_BG_IMAGES)
    ...
    applySelection(currentButtonBgIndex)
    ...
    row.Parent = p
    addCorner(row, 16)
    addStroke(row, Color3.fromRGB(35,35,35), 1)

    local left = Instance.new("TextButton")
    left.Name = "PreviousButton"
    left.Size = UDim2.new(0, 48, 0, 64)
    left.Position = UDim2.new(0, 7, 0.5, -32)
    ...
    right.Name = "NextButton"
    right.Size = UDim2.new(0, 48, 0, 64)
    right.Position = UDim2.new(1, -55, 0.5, -32)
    right.BackgroundColor3 = Color3.fromRGB(12,12,12)
    ...
    viewport.Name = "Carousel"
    viewport.Size = UDim2.new(1, -122, 0, 72)
    viewport.Position = UDim2.new(0, 61, 0.5, -36)
    ...
    local function wrapIndex(index)
        local n = #BUTTON_BG_IMAGES
        return ((index - 1) % n) + 1
    end

    local function refresh()
        windowStart = wrapIndex(currentButtonBgIndex - 2)
        for slot = 1, visibleCount do
            local card = cards[slot]
            local actualIndex = wrapIndex(windowStart + slot - 1)
            card.Image = BUTTON_BG_IMAGES[actualIndex]
            card:SetAttribute("ImageIndex", actualIndex)
            card.LayoutOrder = slot

            local stroke = card:FindFirstChild("SelectionStroke")
            if stroke then
                stroke.Color = (actualIndex == currentButtonBgIndex)
                    and Color3.fromRGB(255,255,255)
                    or Color3.fromRGB(42,42,45)
                stroke.Thickness = (actualIndex == currentButtonBgIndex) and 2.5 or 1
            end
        end
    end
    ...
    for slot = 1, visibleCount do
        ...
        stroke.Color = Color3.fromRGB(42,42,45)
        stroke.Thickness = 1
        stroke.Parent = card

        card.MouseEnter:Connect(function()
            tween(card, {Size = UDim2.new(0, 61, 0, 61)}, 0.12)
        end)
        card.MouseLeave:Connect(function()
            tween(card, {Size = UDim2.new(0, 58, 0, 58)}, 0.12)
        end)
        card.MouseButton1Click:Connect(function()
            local actualIndex = card:GetAttribute("ImageIndex")
            if actualIndex then
                applySelection(actualIndex)
                refresh()
            end
        end)
    ...
    left.MouseButton1Click:Connect(function()
        applySelection(wrapIndex(currentButtonBgIndex - 1))
        refresh()
    end)
    ...
    _AK._setButtonBackgroundImage = applySelection
end
    do
        local saved = loadConfig()
...
-- ============================================================
-- ============================================================
-- BACKGROUNDIMAGESELECTOR (HORIZONTAL CAROUSEL STYLE)
-- ============================================================
do
    local BG_IMAGES = {
        "rbxassetid://127008542588565",
    ...
    -- Image #13 was removed from this selector.
    do
        local savedIndex = tonumber(_configState["BackgroundIndex"])
        if savedIndex then
            if savedIndex == 13 then
                savedIndex = 1
            elseif savedIndex > 13 then
            ...
            _configState["BackgroundIndex"] = savedIndex
        end
    end
    ...
    local function refresh()
        windowStart = wrapIndex(currentBgIndex - 2)
        for slot = 1, visibleCount do
            local card = cards[slot]
            local actualIndex = wrapIndex(windowStart + slot - 1)
            card.Image = BG_IMAGES[actualIndex]
            card:SetAttribute("ImageIndex", actualIndex)
            card.LayoutOrder = slot

            local stroke = card:FindFirstChild("SelectionStroke")
            if stroke then
                stroke.Color = (actualIndex == currentBgIndex)
                    and Color3.fromRGB(255,255,255)
                    or Color3.fromRGB(42,42,45)
                stroke.Thickness = (actualIndex == currentBgIndex) and 2.5 or 1
            end
        end
    end
    ...
    left.MouseButton1Click:Connect(function()
        applySelection(wrapIndex(currentBgIndex - 1))
        refresh()
    end)
...
-- KEYBINDS TAB
-- ============================================================
-- FREEDOM.VS KEYBINDS LOGIC, adapted to the current _AK runtime.
do
    _AK._keybinds = _configState["keybinds"] or {}
    _configState["keybinds"] = _AK._keybinds

    local featureNames = {
        AUTOLEFT="Auto Left", AUTORIGHT="Auto Right", AUTOBAT="Aimbot",
        TPBAT="TP Bat", DROP="Drop", TPDOWN="TP Down",
        CARRYSPEED="Carry Speed", LAGGERSPEED="Lagger Speed",
        LAGGER_CARRY="Lagger Carry",
    }

    local function getKeybind(featureKey)
        _configState["keybinds"] = _configState["keybinds"] or {}
        _AK._keybinds = _configState["keybinds"]
        return _AK._keybinds[featureKey] or ""
    end

    local function setKeybind(featureKey, keyName)
        _configState["keybinds"] = _configState["keybinds"] or {}
        _AK._keybinds = _configState["keybinds"]
        if keyName == "" then _AK._keybinds[featureKey] = nil
        else _AK._keybinds[featureKey] = keyName end
        if _autoSaveConfig then _autoSaveConfig() end
    end

    -- Direct-action registry from the extracted logic, mapped to this GUI.
    _AK._actionRegistry = _AK._actionRegistry or {}
    local function action(key, fn) _AK._actionRegistry[key] = {fn=fn} end

    action("AUTOLEFT", function()
        autoLeftEnabled = not autoLeftEnabled
        if autoLeftEnabled then
        ...
        if _AK._setALOn then _AK._setALOn(autoLeftEnabled) end
    end)
    action("AUTORIGHT", function()
        autoRightEnabled = not autoRightEnabled
        if autoRightEnabled then
        ...
        if _AK._setAROn then _AK._setAROn(autoRightEnabled) end
    end)
    action("AUTOBAT", function()
        if autoBatEnabled then disableAutoBat() else enableAutoBat() end
        if _AK._setABOn then _AK._setABOn(autoBatEnabled) end
    end)
    action("TPBAT", function()
        tpBatEnabled = not tpBatEnabled
        if tpBatEnabled then startTpBat() else stopTpBat() end
    end)
    action("DROP", function() runDrop() end)
    action("TPDOWN", function() runTPFloor() end)
    action("CARRYSPEED", function()
        if laggerToggled then laggerToggled=false; laggerPhase=0; speedMode=true
        else speedMode=not speedMode end
    end)
    action("LAGGERSPEED", function()
    ...
    action("LAGGER_CARRY", function()
        if not laggerToggled then laggerToggled=true; speedMode=false; laggerPhase=2
        elseif laggerPhase==2 then laggerPhase=0 else laggerPhase=2 end
    end)
    local KEYBINDS={Registry={}}
    _AK._KEYBINDS=KEYBINDS
    KEYBINDS.FeatureNames=featureNames
    KEYBINDS.Get=getKeybind
    KEYBINDS.Set=setKeybind

    function KEYBINDS.CreateRow(parent, featureKey, labelText, order)
        local Row=Instance.new("Frame")
        Row.Name="Keybind_"..featureKey; Row.Size=UDim2.new(1,0,0,40)
        Row.BackgroundColor3=Color3.fromRGB(255,255,255); Row.BackgroundTransparency=ROW_ALPHA
        Row.BorderSizePixel=0; Row.LayoutOrder=order; Row.Parent=parent
        addCorner(Row,10); addStroke(Row,Color3.fromRGB(55,55,55),1)
        local rowLabel=Instance.new("TextLabel",Row)
        rowLabel.Size=UDim2.new(0.48,0,0,16); rowLabel.Position=UDim2.new(0,12,0,8)
        rowLabel.BackgroundTransparency=1; rowLabel.Text=labelText; rowLabel.TextColor3=Color3.fromRGB(255,255,255)
        rowLabel.TextSize=12; rowLabel.Font=Enum.Font.GothamBold; rowLabel.TextXAlignment=Enum.TextXAlignment.Left; rowLabel.ZIndex=5
        local ul=Instance.new("Frame",Row); ul.Size=UDim2.new(0,math.min(#labelText*5.8,150),0,1); ul.Position=UDim2.new(0,12,0,25)
        ul.BackgroundColor3=Color3.fromRGB(255,255,255); ul.BackgroundTransparency=0.6; ul.BorderSizePixel=0; addCorner(ul,1)
        local keyBtn=Instance.new("TextButton",Row); keyBtn.Active=true; keyBtn.Size=UDim2.new(0,80,0,24); keyBtn.Position=UDim2.new(1,-116,0.5,-12)
        keyBtn.BackgroundColor3=Color3.fromRGB(50,50,50); keyBtn.BorderSizePixel=0; keyBtn.TextColor3=Color3.fromRGB(255,255,255); keyBtn.TextSize=10; keyBtn.Font=Enum.Font.GothamBold
        keyBtn.Text=getKeybind(featureKey)~="" and getKeybind(featureKey) or "NONE"; keyBtn.ZIndex=6; keyBtn.AutoButtonColor=false; addCorner(keyBtn,5)
        local keyStroke=addStroke(keyBtn,Color3.fromRGB(80,80,80),1.5)
        local clearBtn=Instance.new("TextButton",Row); clearBtn.Active=true; clearBtn.Size=UDim2.new(0,24,0,24); clearBtn.Position=UDim2.new(1,-28,0.5,-12)
        clearBtn.BackgroundColor3=Color3.fromRGB(80,80,80); clearBtn.BorderSizePixel=0; clearBtn.Text="X"; clearBtn.TextColor3=Color3.fromRGB(255,255,255); clearBtn.TextSize=10; clearBtn.Font=Enum.Font.GothamBold; clearBtn.ZIndex=6; clearBtn.AutoButtonColor=false; addCorner(clearBtn,5); addStroke(clearBtn,Color3.fromRGB(150,150,150),1)
        local listening=false; local listenConn
        local function stopListening()
            listening=false; if listenConn then listenConn:Disconnect(); listenConn=nil end
            keyBtn.Text=getKeybind(featureKey)~="" and getKeybind(featureKey) or "NONE"; keyBtn.BackgroundColor3=Color3.fromRGB(50,50,50); keyStroke.Color=Color3.fromRGB(80,80,80)
        end
        local function startListening()
            if listening then return end; listening=true; keyBtn.Text="PRESS KEY..."; keyBtn.BackgroundColor3=Color3.fromRGB(100,100,100); keyStroke.Color=Color3.fromRGB(200,200,200)
            if listenConn then listenConn:Disconnect() end
            listenConn=UIS.InputBegan:Connect(function(input)
                if not listening then return end
                if input.UserInputType==Enum.UserInputType.Keyboard or string.find(input.UserInputType.Name,"Gamepad") then
                    local key=input.KeyCode.Name
                    if key~="Unknown" then setKeybind(featureKey,key); keyBtn.Text=key; stopListening() end
                end
            end)
        end
        keyBtn.Activated:Connect(function() if listening then stopListening() else startListening() end end)
        clearBtn.Activated:Connect(function() stopListening(); setKeybind(featureKey,""); keyBtn.Text="NONE" end)
        table.insert(KEYBINDS.Registry,{key=featureKey,btn=keyBtn}); hoverRow(Row,ROW_ALPHA); return Row
    end

    local p=tabPages["KEYBINDS"]
    if p then
        sectionLabel(p,"KEYBINDS",1)
        local info=Instance.new("TextLabel"); info.Name="KeybindInfo"; info.Size=UDim2.new(1,0,0,34); info.BackgroundColor3=Color3.fromRGB(255,255,255); info.BackgroundTransparency=0.94; info.BorderSizePixel=0; info.LayoutOrder=2; info.Parent=p
        info.Text="Tap a key box, then press a keyboard/gamepad key. X clears the bind."; info.TextColor3=Color3.fromRGB(170,170,170); info.TextSize=10; info.Font=Enum.Font.Gotham; info.TextWrapped=true; info.TextXAlignment=Enum.TextXAlignment.Left; info.ZIndex=5; addCorner(info,8); addStroke(info,Color3.fromRGB(45,45,45),1)
        local order=3
        for featureKey,name in pairs(featureNames) do KEYBINDS.CreateRow(p,featureKey,name,order); order=order+1 end
    end

    -- Extracted dispatch order: direct action first, toggle fallback second.
    if _AK._keybindListener then pcall(function() _AK._keybindListener:Disconnect() end) end
    _AK._keybindListener=UIS.InputBegan:Connect(function(input)
        if input.UserInputType~=Enum.UserInputType.Keyboard and not string.find(input.UserInputType.Name,"Gamepad") then return end
        local keyName=input.KeyCode.Name; if keyName=="Unknown" then return end
        local binds=_AK._keybinds; if not binds then return end
        for featureKey,boundKey in pairs(binds) do
            if boundKey==keyName then
                local direct=_AK._actionRegistry[featureKey]
                if direct and type(direct.fn)=="function" then pcall(direct.fn); break end
                for _,entry in ipairs(_toggleRegistry) do
                    if entry.key==featureKey then
                        local newState=not (_configState[featureKey]==true)
                        pcall(function() entry.setState(newState) end); _configState[featureKey]=newState
                        if entry.callback then pcall(entry.callback,newState) end
                        if _autoSaveConfig then _autoSaveConfig() end
                        break
                    end
                end
    ...
    KEYBINDS.StartInputListener=function() return _AK._keybindListener end
end
