...
-- ============================================================
-- CUSTOM TOOLS + CUSTOM TOOL SOUND
-- Adapted from the supplied extracted component for this hub's
-- current GUI helpers / VISUALS tab.
-- ============================================================
do
    local ASSETS = {
            DiamondSword      = { cat = "Bat",    label = "Diamond Sword",
                                  ...
                                  -- view = distanza della camera nell'anteprima:
                                  -- piu' alta, piu' piccolo appare l'oggetto
                                  view = 4.9 },
    ...
    _G.AmbitiousCustomToolAssets = ASSETS
    _G.AmbitiousCustomToolSkins = _G.AmbitiousCustomToolSkins
        or { Bat = "DiamondSword", Medusa = "GoldenDesertEagle" }

    local function ctCategory(tool)
        if not tool or not tool:IsA("Tool") then return nil end
        local n = string.lower(tool.Name or "")
        if n == "bat" or n:find("bat", 1, true) then return "Bat" end
        if n == "medusa" or n:find("medusa", 1, true) then return "Medusa" end
        return nil
    end

    local function skinFor(cat)
        local key = _G.AmbitiousCustomToolSkins[cat]
        if key and ASSETS[key] and ASSETS[key].cat == cat then
            return key
        end
        return cat == "Bat" and "DiamondSword" or "GoldenDesertEagle"
    end

    local toolEnabled = _G.AmbitiousCustomToolsEnabled == true
    local soundEnabled = _G.AmbitiousCustomSoundsEnabled == true
    local tracked = setmetatable({}, {__mode="k"})
    local soundTracked = setmetatable({}, {__mode="k"})
    local watchers = setmetatable({}, {__mode="k"})

    local function buildReplica(handle, key)
        if not handle or not handle.Parent then return end
        local old = handle:FindFirstChild("LocalReplica")
        if old then old:Destroy() end

        local data = ASSETS[key]
    ...
    local function restoreTool(entry)
        if not entry then return end
        for obj, old in pairs(entry.originalTransparency or {}) do
            if obj and obj.Parent then
                pcall(function() obj.Transparency = old end)
            end
        end
    ...
    local function setupTool(tool)
        if not tool or not tool.Parent or tracked[tool] then return end
        local cat = ctCategory(tool)
        if not cat then return end

        local handle = tool:FindFirstChild("Handle")
        if not handle then return end

        local entry = {tool=tool, cat=cat, originalTransparency={}}
        tracked[tool] = entry

        for _, obj in ipairs(tool:GetDescendants()) do
            if obj:IsA("BasePart") or obj:IsA("Decal") or obj:IsA("Texture") then
                entry.originalTransparency[obj] = obj.Transparency
        ...
        buildReplica(handle, skinFor(cat))

        entry.ancestry = tool.AncestryChanged:Connect(function(_, parent)
            if not parent then
                tracked[tool] = nil
            end
        end)
    ...
    local function refreshTools()
        ...
        local backpack = _LP:FindFirstChildOfClass("Backpack")
    ...
    local function setCustomTools(on)
        toolEnabled = on and true or false
        _G.AmbitiousCustomToolsEnabled = toolEnabled
        ...
        if toolEnabled then
            ...
            for tool, entry in pairs(tracked) do
                restoreTool(entry)
                if entry.ancestry then pcall(function() entry.ancestry:Disconnect() end) end
                tracked[tool] = nil
            end
        end
    end

    _G.AmbitiousSetCustomToolSkin = function(cat, key)
        if not ASSETS[key] or ASSETS[key].cat ~= cat then return end
        _G.AmbitiousCustomToolSkins[cat] = key
        if toolEnabled then
            for tool, entry in pairs(tracked) do
                if entry.cat == cat and tool.Parent then
                    local handle = tool:FindFirstChild("Handle")
                    if handle then
                        buildReplica(handle, key)
                    end
                end
            end
        end
        _configState["customToolSkin_" .. cat] = key
        _autoSaveConfig()
    end

    local function unwatch(container)
        local list = watchers[container]
        ...
        watchers[container] = {
            container.ChildAdded:Connect(function(child)
                if child:IsA("Tool") then
                    task.defer(function()
                        if toolEnabled then setupTool(child) end
                    end)
                end
            end)
        }
    ...
    _LP.CharacterAdded:Connect(function(char)
        ...
        task.wait(0.25)
        if toolEnabled then watchAll() end
        if soundEnabled then
            task.wait(0.15)
            if _G.AmbitiousCustomSounds then
                _G.AmbitiousCustomSounds.setEnabled(true)
            end
        end
    end)

    _LP.ChildAdded:Connect(function(child)
        if toolEnabled and child:IsA("Backpack") then
            watch(child)
            refreshTools()
        end
    end)
    ...
    _G.AmbitiousCustomTools = {
        setEnabled = setCustomTools,
        isEnabled = function() return toolEnabled end,
    }
    ...
    local function playCustomSound(entry)
        local snd = entry.custom
        local data = entry.data
        if not snd or not snd.Parent or not data then return end
        pcall(function()
            snd:Stop()
            snd.TimePosition = data.skip or 0
            snd.Volume = 1
            snd:Play()
        end)
        if data.doublePlay and data.delay then
            ...
            entry.timer = task.delay(data.delay, function()
                if not soundEnabled or not snd.Parent then return end
                pcall(function()
                    snd:Stop()
                    snd.TimePosition = data.skip or 0
                    snd.Volume = 1
    ...
    local function setupSoundTool(tool)
        if not tool or not tool.Parent or soundTracked[tool] then return end
        local cat = ctCategory(tool)
        if not cat or not SOUNDS[cat] then return end
        local handle = tool:FindFirstChild("Handle")
        if not handle then return end

        local custom = handle:FindFirstChild("CustomSound")
        if custom then custom:Destroy() end

        custom = Instance.new("Sound")
        custom.Name = "CustomSound"
        custom.SoundId = SOUNDS[cat].id
        custom.Volume = 1
        custom.Looped = false
        custom.RollOffMode = Enum.RollOffMode.Inverse
        custom.MaxDistance = 1000
        custom.MinDistance = 1
        custom.Parent = handle

        local entry = {tool=tool, custom=custom, data=SOUNDS[cat], muted={}}
        soundTracked[tool] = entry

        local function hook(obj)
            if not obj:IsA("Sound") or obj == custom or hooked[obj] then return end
            hooked[obj] = true
            entry.muted[obj] = obj.Volume

            local function trigger()
                if not soundEnabled or not obj.Parent then return end
                if obj.Playing or obj.TimePosition > 0 then
                    pcall(function()
                        obj.Volume = 0
                        obj:Stop()
                    end)
                    playCustomSound(entry)
                end
            end

            entry["c1_" .. tostring(obj)] = obj:GetPropertyChangedSignal("Playing"):Connect(trigger)
            entry["c2_" .. tostring(obj)] = obj:GetPropertyChangedSignal("TimePosition"):Connect(trigger)
            entry["c3_" .. tostring(obj)] = obj.AncestryChanged:Connect(function(_, parent)
        ...
        for _, obj in ipairs(tool:GetDescendants()) do hook(obj) end
        entry.desc = tool.DescendantAdded:Connect(hook)
        entry.ancestry = tool.AncestryChanged:Connect(function(_, parent)
            if not parent then
                for obj, vol in pairs(entry.muted) do
                    if obj and obj.Parent then pcall(function() obj.Volume = vol end) end
                    hooked[obj] = nil
                end
                if entry.custom then entry.custom:Destroy() end
                soundTracked[tool] = nil
            end
        end)
    end

    local function setCustomSounds(on)
        soundEnabled = on and true or false
        _G.AmbitiousCustomSoundsEnabled = soundEnabled
        ...
        if soundEnabled then
            ...
            for _, container in ipairs({backpack, char}) do
                if container then
                    for _, obj in ipairs(container:GetChildren()) do
                        if obj:IsA("Tool") then setupSoundTool(obj) end
                    end
                end
            end
        else
            for tool, entry in pairs(soundTracked) do
                if entry.timer then pcall(task.cancel, entry.timer) end
                for obj, vol in pairs(entry.muted) do
                    hooked[obj] = nil
                    if obj and obj.Parent then pcall(function() obj.Volume = vol end) end
                end
                for k, c in pairs(entry) do
                    if typeof(c) == "RBXScriptConnection" then pcall(function() c:Disconnect() end) end
                end
                if entry.custom then pcall(function() entry.custom:Destroy() end) end
                soundTracked[tool] = nil
            end
        end
    end
    ...
    _G.AmbitiousCustomSounds = {
        setEnabled = setCustomSounds,
        isEnabled = function() return soundEnabled end,
    }

    if toolEnabled then watchAll() end
    if soundEnabled then setCustomSounds(true) end
end
...
;(function()
    ...
    -- CUSTOM TOOLS toggle must be directly below JUMP MODE.
    _regToggle(p, "CUSTOM TOOLS", _G.AmbitiousCustomToolsEnabled == true, 36, "CUSTOMTOOLS", function(on)
        if _G.AmbitiousCustomTools then
            _G.AmbitiousCustomTools.setEnabled(on)
        end
    end)

    -- Selector directly below CUSTOM TOOLS.
    do
        local Row = Instance.new("Frame")
        Row.Size = UDim2.new(1,0,0,40)
        Row.BackgroundColor3 = Color3.fromRGB(255,255,255)
        ...
        label.Size = UDim2.new(0.58,0,0,16)
        label.Position = UDim2.new(0,12,0,8)
        label.BackgroundTransparency = 1
        label.Text = "CUSTOM TOOLS SELECTOR"
        ...
        pick.Size = UDim2.new(0,68,0,24)
        pick.Position = UDim2.new(1,-80,0.5,-12)
        pick.BackgroundColor3 = Color3.fromRGB(255,255,255)
        pick.BackgroundTransparency = 0.05
        ...
        addCorner(pick, 8)
        addStroke(pick, Color3.fromRGB(120,120,120), 1)
        ...
        local function makeCard(parent, key, def, x)
            local card = Instance.new("Frame", parent)
            card.Size = UDim2.new(0,124,0,145)
            ...
            card.ZIndex = 4
            addCorner(card, 12)
            local st = addStroke(card, Color3.fromRGB(55,55,55), 1.2)

            local vp = Instance.new("ViewportFrame", card)
            vp.Size = UDim2.new(1,-12,0,92)
            ...
            local part = Instance.new("Part", vp)
            part.Anchored = true
            part.CanCollide = false
            part.Size = Vector3.new(1,1,1)
            ...
            cam.FieldOfView = 45
            cam.CFrame = CFrame.new(Vector3.new(0,(def.view or 4)*0.35,def.view or 4), Vector3.zero)
            vp.CurrentCamera = cam

            local name = Instance.new("TextLabel", card)
            name.Size = UDim2.new(1,-10,0,36)
            name.Position = UDim2.new(0,5,0,102)
            ...
            local function refresh()
                local selected = _G.AmbitiousCustomToolSkins[def.cat] == key
                tween(st, {Color = selected and Color3.fromRGB(255,255,255) or Color3.fromRGB(55,55,55),
                    Transparency = selected and 0 or 0.4,
                    Thickness = selected and 2 or 1.2}, 0.12)
            end
            hit.MouseButton1Click:Connect(function()
                if _G.AmbitiousSetCustomToolSkin then
                    _G.AmbitiousSetCustomToolSkin(def.cat,key)
                end
                refresh()
            end)
            return {part=part, refresh=refresh}
        end
        ...
        pick.MouseButton1Click:Connect(function()
            ...
            popup.Name = "CustomToolsSelectorPopup"
            popup.Size = UDim2.new(0,276,0,330)
            popup.Position = UDim2.new(0.5,-138,0.5,-165)
            popup.BackgroundColor3 = Color3.fromRGB(10,10,12)
            popup.BorderSizePixel = 0
            popup.ZIndex = 100
            popup.Parent = PGui
            addCorner(popup, 14)
            addStroke(popup, Color3.fromRGB(90,90,105), 1.2, 0.2)

            local title = Instance.new("TextLabel", popup)
            title.Size = UDim2.new(1,-44,0,34)
            title.Position = UDim2.new(0,12,0,6)
            title.BackgroundTransparency = 1
            title.Text = "CUSTOM TOOLS SELECTOR"
            ...
            local cats = {
                {key="Bat", text="BAT"},
                {key="Medusa", text="MEDUSA"}
            }

            local tabs = Instance.new("Frame", popup)
            ...
            local function showCat(cat)
                ...
                for k,b in pairs(tabButtons) do
                    b.BackgroundColor3 = (k==cat) and Color3.fromRGB(255,255,255) or Color3.fromRGB(25,25,30)
                    b.TextColor3 = (k==cat) and Color3.fromRGB(0,0,0) or Color3.fromRGB(180,180,180)
                end
                for k,page in pairs(pages) do page.Visible = (k==cat) end
            end

            for i,c in ipairs(cats) do
                local b = Instance.new("TextButton", tabs)
                b.Size = UDim2.new(0.5,0,1,0)
                b.Position = UDim2.new((i-1)*0.5,0,0,0)
                b.BackgroundColor3 = i==1 and Color3.fromRGB(255,255,255) or Color3.fromRGB(25,25,30)
                b.BorderSizePixel = 0
                b.Text = c.text
                b.TextColor3 = i==1 and Color3.fromRGB(0,0,0) or Color3.fromRGB(180,180,180)
                b.TextSize = 10
                b.Font = Enum.Font.GothamBlack
                b.ZIndex = 103
                tabButtons[c.key]=b
                addCorner(b,8)
                b.MouseButton1Click:Connect(function() showCat(c.key) end)

                local page = Instance.new("Frame", popup)
                page.Size = UDim2.new(1,-24,0,150)
                page.Position = UDim2.new(0,12,0,92)
                page.BackgroundTransparency = 1
                page.ZIndex = 102
                page.Visible = i==1
                pages[c.key]=page
                ...
                local keys={}
                for key,def in pairs(ASSETS) do
                    if def.cat==c.key then keys[#keys+1]={key=key,def=def} end
                end
                table.sort(keys,function(a,b) return a.key < b.key end)

                local cards={}
                for j,item in ipairs(keys) do
                    cards[#cards+1]=makeCard(page,item.key,item.def,(j-1)*128)
                end

                local oldRefresh = cards
                for _,entry in ipairs(oldRefresh) do entry.refresh() end
            end
            ...
            spinConn = RunService.RenderStepped:Connect(function(dt)
                ...
                for _,page in pairs(pages) do
                    for _,obj in ipairs(page:GetChildren()) do
                        local vp = obj:FindFirstChildOfClass("ViewportFrame")
                        if vp then
                            local part = vp:FindFirstChildWhichIsA("Part")
                            if part then part.CFrame = part.CFrame * CFrame.Angles(0,dt*1.2,0) end
                        end
                    end
                end
            end)
    ...
    -- CUSTOM TOOL SOUND toggle directly below selector.
    _regToggle(p, "CUSTOM TOOLS SOUND", _G.AmbitiousCustomSoundsEnabled == true, 38, "CUSTOMTOOLSSOUND", function(on)
        if _G.AmbitiousCustomSounds then
            _G.AmbitiousCustomSounds.setEnabled(on)
        end
    end)
...
_AK.cfgPack = {
    ...
    NUKEOPT = "d9", ANTIDIE = "e3", SAFEMODE = "e4", CUSTOMTOOLS = "f1", CUSTOMTOOLSSOUND = "f2",
}
_AK.cfgUpack = {}
