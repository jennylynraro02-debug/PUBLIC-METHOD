...
-- Horizontal carousel style (rounded image cards)
-- ============================================================
do
    local BG_IMAGES = {
    ...
    if p then
        ...
        row.Size = UDim2.new(1, 0, 0, 178)
        row.BackgroundColor3 = Color3.fromRGB(8, 8, 9)
        row.BackgroundTransparency = 0.08
        row.BorderSizePixel = 0
        row.LayoutOrder = 37
        row.Parent = p
        addCorner(row, 18)
        addStroke(row, Color3.fromRGB(65,65,70), 1.2)
        ...
        sub.TextColor3 = Color3.fromRGB(145,145,150)
        sub.TextSize = 9
        ...
        -- The screenshot style is a single horizontal strip of rounded cards.
        local scroll = Instance.new("ScrollingFrame")
        scroll.Name = "BackgroundCarousel"
        scroll.Size = UDim2.new(1, -20, 0, 105)
        scroll.Position = UDim2.new(0, 10, 0, 51)
        scroll.BackgroundColor3 = Color3.fromRGB(3,3,4)
        ...
        local function refresh()
            sub.Text = string.format(
                "SELECTED  %02d / %02d   •   SWIPE TO BROWSE",
            ...
            for i, card in ipairs(cards) do
                ...
                if stroke then
                    stroke.Color = (i == currentBgIndex)
                        ...
                        or Color3.fromRGB(58,58,64)
                    stroke.Thickness = (i == currentBgIndex) and 3 or 1.2
                    stroke.Transparency = (i == currentBgIndex) and 0 or 0.25
                end

                local shade = card:FindFirstChild("Shade")
                if shade then
                    shade.BackgroundTransparency = (i == currentBgIndex) and 0.78 or 0.88
                end
            end
        end
        ...
        for i, imageId in ipairs(BG_IMAGES) do
            ...
            shade.BackgroundTransparency = 0.88
            shade.BorderSizePixel = 0
            shade.ZIndex = 8
            shade.Parent = card
            addCorner(shade, 18)

            -- Small selection mark; no numeric ID labels, matching the reference style.
            ...
            card.MouseButton1Click:Connect(function()
                ...
                -- Bring the selected card into view in the horizontal strip.
                pcall(function()
                    local x = math.max(0, card.AbsolutePosition.X - scroll.AbsolutePosition.X - 20)
                    scroll.CanvasPosition = Vector2.new(x, 0)
                end)
            end)
        ...
        local function refreshDots()
            for i, card in ipairs(cards) do
                local dot = card:FindFirstChild("SelectedDot")
                if dot then
                    dot.BackgroundTransparency = (i == currentBgIndex) and 0 or 1
                end
            end
        end
