...
-- ============================================================
-- CUSTOM TOOLS SUPPORT HELPERS
-- Adapted for the current BLOCK DUELS hub.
-- ============================================================
local CT_Trans = CT_Trans or {}
do
    local claims = setmetatable({}, {__mode = "k"})
    function CT_Trans.claim(obj, owner, value)
        if not obj or not obj.Parent then return end
        claims[obj] = claims[obj] or {}
        claims[obj][owner] = value
        pcall(function() obj.Transparency = value end)
    end
    function CT_Trans.release(obj, owner)
        local c = claims[obj]
        if not c then return end
        c[owner] = nil
        local value = nil
        for _, v in pairs(c) do value = v break end
        if value == nil then
            claims[obj] = nil
...
local function isReplicaStuff(inst)
    if not inst then return false end
    local p = inst.Parent
    while p do
        if p.Name == "LocalReplica" then return true end
        p = p.Parent
    end
    return false
end

local function ctMatchItem(tool)
    if not tool or not tool:IsA("Tool") then return nil end
    local n = string.lower(tool.Name or "")
    if n:find("medusa", 1, true) then
        return {key = "Medusa"}
    end
    if n:find("bat", 1, true) then
        return {key = "Bat"}
    end
    -- Some versions rename the actual tool; inspect descendants too.
    for _, d in ipairs(tool:GetDescendants()) do
        local dn = string.lower(d.Name or "")
        if dn:find("medusa", 1, true) then
            return {key = "Medusa"}
        elseif dn:find("bat", 1, true) then
            return {key = "Bat"}
        end
    end
    return nil
...
local CT_JANITORS = CT_JANITORS or {}
local _LP = LP
local saveAmbitiousConfig = function()
    if _autoSaveConfig then
        _autoSaveConfig()
    end
end

    -- ##                     CUSTOM TOOLS                       ##
    -- ## Bat -> Diamond Sword  Medusa's Head -> Golden Desert Eagle ##
    -- ############################################################
    do
        -- Catalogo skin. "cat" e' la categoria di oggetto su cui si applica.
        local ASSETS = {
        ...
        -- Skin attualmente scelta per ogni categoria.
        ...
        local enabled       = false
        local tracked       = setmetatable({}, { __mode = "k" })  -- [tool] = { hidden, conns }
        local watchers      = setmetatable({}, { __mode = "k" })
        local pendingHandle = setmetatable({}, { __mode = "k" })  -- [tool] = conn
        local teardown
        ...
        local function buildReplica(handle, mode)
            ...
            -- Scala e impugnatura vengono dal catalogo, non da un if per skin
        ...
        -- Nasconde (NON distrugge) solo la grafica: parti, decal e texture.
        -- Suoni, script e componenti di gioco restano intatti.
        -- Idempotente di proposito: il watchdog lo richiama sulle stesse
        -- parti per ri-prenotare la trasparenza se e' stata persa.
        ...
        _LP.CharacterAdded:Connect(function(char)
            ...
            -- pulizia totale: il vecchio Character e il vecchio Backpack sono morti
        ...
        -- ------------------------------------------------------------
        -- WATCHDOG INTEGRITA'
        -- Copre i due bug segnalati:
        --  1) la replica custom sparisce e resta solo l'arma originale
        --     (tool ri-parentato, Handle sostituito, replica distrutta);
        --  2) l'arma originale ricompare sopra la replica
        --     (trasparenza sovrascritta dal gioco -> ri-nascosta).
        -- ------------------------------------------------------------
        local function rebuildIfNeeded(tool, entry, deep)
            ...
            -- Ripassata completa solo se la replica era rotta o su richiesta
            if broken or deep then
                for _, d in ipairs(tool:GetDescendants()) do hideNode(entry, d) end
            end
        end

        -- deep = ripassa anche tutti i descendant dei tool (piu' costoso).
        -- Il giro leggero controlla solo che la replica esista: la
        -- trasparenza e' gia' coperta dai guard + sweep di CT_Trans.
        local function integrityPass(deep)
            if not enabled then return end
            -- ri-aggancia i container (Backpack ricreato, respawn, ecc.)
            watchAll()
        ...
        _G.AmbitiousCustomToolsRecheck = function() pcall(integrityPass, true) end

        -- Cambio skin: aggiorna la scelta e ricostruisce subito le repliche
        -- gia' agganciate, senza dover spegnere e riaccendere il modulo.
        ...
        task.spawn(function()
            ...
            while true do
                ...
                if enabled then
                    n = n + 1
                    pcall(integrityPass, (n % 5) == 0)   -- deep ogni 2s
                end
            end
        end)

        -- Riapplica subito quando equipaggi/sganci un'arma, senza aspettare
        -- il giro del watchdog.
        ...
        _G.AmbitiousCustomTools = {
            setEnabled = function(on)
                ...
                enabled = on
                _G.AmbitiousCustomToolsEnabled = on
                if on then
                    watchAll()
...
-- ============================================================
-- CUSTOM TOOLS VISUALS UI
-- Placement:
--   JUMP MODE
--   CUSTOM TOOLS toggle
--   CUSTOM TOOLS SELECTOR
-- ============================================================
;(function()
    local p = tabPages["VISUALS"]
    ...
    local customToolsOn = saved["CUSTOMTOOLS"] == true
        or _G.AmbitiousCustomToolsEnabled == true

    local _, setCustomToolsOn = _regToggle(
        p,
        "CUSTOM TOOLS",
        customToolsOn,
        35.5,
        "CUSTOMTOOLS",
        function(on)
            if _G.AmbitiousCustomTools
                and _G.AmbitiousCustomTools.setEnabled then
                _G.AmbitiousCustomTools.setEnabled(on)
            end
        end
    )

    _AK._setCustomToolsOn = setCustomToolsOn

    -- Selector row directly below the CUSTOM TOOLS toggle.
    local row = Instance.new("Frame")
    row.Name = "CustomToolsSelector"
    row.Size = UDim2.new(1, 0, 0, 190)
    row.BackgroundColor3 = Color3.fromRGB(255,255,255)
    row.BackgroundTransparency = 0.93
    row.BorderSizePixel = 0
    row.LayoutOrder = 35.6
    row.Parent = p
    addCorner(row, 10)
    addStroke(row, Color3.fromRGB(55,55,55), 1)
    ...
    selected.Size = UDim2.new(1, -24, 0, 14)
    selected.Position = UDim2.new(0, 12, 0, 27)
    selected.BackgroundTransparency = 1
    selected.TextColor3 = Color3.fromRGB(155,155,155)
    selected.TextSize = 9
    ...
    grid.Name = "ToolGrid"
    grid.Size = UDim2.new(1, -20, 0, 145)
    grid.Position = UDim2.new(0, 10, 0, 45)
    ...
    gridLayout.CellSize = UDim2.new(0, 118, 0, 64)
    ...
    local entries = {
        {key="DiamondSword", cat="Bat", label="DIAMOND SWORD"},
        {key="Katana", cat="Bat", label="KATANA"},
        {key="Skull", cat="Medusa", label="SKULL"},
        {key="GoldenDesertEagle", cat="Medusa", label="GOLDEN DESERT EAGLE"},
    }

    local cards = {}

    local function refresh()
        local bat = _G.AmbitiousCustomToolSkins
            and _G.AmbitiousCustomToolSkins.Bat
            or "DiamondSword"
        local medusa = _G.AmbitiousCustomToolSkins
            and _G.AmbitiousCustomToolSkins.Medusa
            or "GoldenDesertEagle"
        ...
        for _, e in ipairs(cards) do
            local current = (e.cat == "Bat" and bat or medusa)
            local on = current == e.key
            e.stroke.Color = on
                and Color3.fromRGB(255,255,255)
                or Color3.fromRGB(65,65,65)
            e.stroke.Thickness = on and 2 or 1
            e.card.BackgroundTransparency = on and 0.03 or 0.15
        end
    end

    for i, def in ipairs(entries) do
        local card = Instance.new("TextButton")
        card.Name = "CustomTool_" .. def.key
        ...
        local st = addStroke(card, Color3.fromRGB(65,65,65), 1)
        local item = {card=card, stroke=st, key=def.key, cat=def.cat}
        table.insert(cards, item)

        card.MouseEnter:Connect(function()
            tween(card, {BackgroundTransparency = 0.02}, 0.12)
        end)
        ...
        card.MouseButton1Click:Connect(function()
            if _G.AmbitiousSetCustomToolSkin then
                _G.AmbitiousSetCustomToolSkin(def.cat, def.key)
            end
            refresh()
        end)
    end
    ...
    -- Restore saved skin choices when the hub starts.
    task.defer(function()
        local cfg = loadConfig()
        local bat = cfg["customToolBat"]
        local medusa = cfg["customToolMedusa"]
        if bat and _G.AmbitiousSetCustomToolSkin then
            pcall(_G.AmbitiousSetCustomToolSkin, "Bat", bat)
        end
        if medusa and _G.AmbitiousSetCustomToolSkin then
            pcall(_G.AmbitiousSetCustomToolSkin, "Medusa", medusa)
        end
        refresh()
    end)

    -- Persist selector choices.
    local oldSet = _G.AmbitiousSetCustomToolSkin
    if oldSet and not _G._CustomToolsSaveWrapped then
        _G._CustomToolsSaveWrapped = true
        _G.AmbitiousSetCustomToolSkin = function(cat, key)
            oldSet(cat, key)
            if cat == "Bat" then
                _configState["customToolBat"] = key
            elseif cat == "Medusa" then
                _configState["customToolMedusa"] = key
            end
            _autoSaveConfig()
            refresh()
        end
    ...
    -- Apply the saved toggle after the UI exists.
    task.defer(function()
        if _G.AmbitiousCustomTools and _G.AmbitiousCustomTools.setEnabled then
            _G.AmbitiousCustomTools.setEnabled(customToolsOn)
        end
        if setCustomToolsOn then setCustomToolsOn(customToolsOn) end
    end)
end)()
