...
do
    ...
    -- TITLE STYLE SYSTEM (from TITLE_STYLE_EXTRACTED.lua)
    local TitleMain = Instance.new("ImageLabel", HeaderFrame)
    TitleMain.Name = "TitleMain"
    TitleMain.AnchorPoint = Vector2.new(0.5, 0)
    TitleMain.Position = UDim2.new(0.5, 0, 0, 10)
    TitleMain.Size = UDim2.new(0, 255, 0, 76)
    TitleMain.BackgroundTransparency = 1
    TitleMain.Image = "rbxassetid://128938872032759"
    TitleMain.ScaleType = Enum.ScaleType.Fit
    TitleMain.ZIndex = 6
    TitleMain.Active = false

    local TITLE_STYLES = {
        {name = "TITLE 1", image = "rbxassetid://128938872032759", scale = 1,    prev = 2.5},
        {name = "TITLE 2", image = "rbxassetid://80924330621180",  scale = 0.64, prev = 1.3},
        {name = "TITLE 3", image = "rbxassetid://105333739149934", scale = 0.56, prev = 1.3},
    }

    local titleStyleIndex = math.clamp(
        math.floor(tonumber(_configState["titleStyle"]) or 1),
        1,
        #TITLE_STYLES
    )

    local function applyTitleStyle(index)
        titleStyleIndex = math.clamp(
            math.floor(tonumber(index) or 1),
            1,
            #TITLE_STYLES
        )

        local st = TITLE_STYLES[titleStyleIndex]
        if not st then return end

        TitleMain.Image = st.image

        -- Keep every title style inside the 295px panel header while
        -- preserving the extracted style's relative scale.
        local maxW = 255
        local maxH = 70
        local rel = tonumber(st.scale) or 1
        local w = math.floor(maxW * rel + 0.5)
        local h = math.floor(maxH * rel + 0.5)

        TitleMain.Size = UDim2.new(0, w, 0, h)
        TitleMain.Position = UDim2.new(0.5, 0, 0, 8 + math.floor((70 - h) / 2))

        _configState["titleStyle"] = titleStyleIndex
        _autoSaveConfig()
    end

    applyTitleStyle(titleStyleIndex)
...
;(function()
    ...
    -- Placed directly below SKY THEME and uses the same selector UI.
    -- ============================================================
    do
        local titleStyleRow = Instance.new("Frame")
        titleStyleRow.Name = "TitleStyleRow"
        titleStyleRow.Size = UDim2.new(1, 0, 0, 42)
        titleStyleRow.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        titleStyleRow.BackgroundTransparency = ROW_ALPHA
        titleStyleRow.BorderSizePixel = 0
        titleStyleRow.LayoutOrder = 93.7
        titleStyleRow.Parent = p
        addCorner(titleStyleRow, 10)
        addStroke(titleStyleRow, Color3.fromRGB(55, 55, 55), 1)

        local titleStyleLabel = Instance.new("TextLabel", titleStyleRow)
        titleStyleLabel.Name = "Label"
        titleStyleLabel.BackgroundTransparency = 1
        titleStyleLabel.Text = "TITLE STYLE"
        titleStyleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
        titleStyleLabel.TextSize = 12
        titleStyleLabel.Font = Enum.Font.GothamBold
        titleStyleLabel.TextXAlignment = Enum.TextXAlignment.Left
        titleStyleLabel.Position = UDim2.new(0, 12, 0, 8)
        titleStyleLabel.Size = UDim2.new(0.5, 0, 0, 16)
        titleStyleLabel.ZIndex = 5

        local titleStyleUl = Instance.new("Frame", titleStyleRow)
        titleStyleUl.Size = UDim2.new(0, 70, 0, 1)
        titleStyleUl.Position = UDim2.new(0, 12, 0, 25)
        titleStyleUl.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        titleStyleUl.BackgroundTransparency = 0.5
        titleStyleUl.BorderSizePixel = 0
        addCorner(titleStyleUl, 1)

        local titleStyleCont = Instance.new("Frame", titleStyleRow)
        titleStyleCont.AnchorPoint = Vector2.new(1, 0.5)
        titleStyleCont.Position = UDim2.new(1, -10, 0.5, 0)
        titleStyleCont.Size = UDim2.new(0, 170, 0, 26)
        titleStyleCont.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
        titleStyleCont.BorderSizePixel = 0
        titleStyleCont.ZIndex = 5
        addCorner(titleStyleCont, 5)
        addStroke(titleStyleCont, Color3.fromRGB(55, 55, 55), 1)

        local titleStyleArW = 28

        local titleStylePrev = Instance.new("TextButton", titleStyleCont)
        titleStylePrev.Size = UDim2.new(0, titleStyleArW, 1, 0)
        titleStylePrev.Position = UDim2.new(0, 0, 0, 0)
        titleStylePrev.BackgroundTransparency = 1
        titleStylePrev.BorderSizePixel = 0
        titleStylePrev.Text = "<"
        titleStylePrev.TextColor3 = Color3.fromRGB(200, 200, 200)
        titleStylePrev.Font = Enum.Font.GothamBold
        titleStylePrev.TextSize = 14
        titleStylePrev.ZIndex = 6

        local titleStyleValue = Instance.new("TextLabel", titleStyleCont)
        titleStyleValue.Name = "TitleStyleValue"
        titleStyleValue.Position = UDim2.new(0, titleStyleArW, 0, 0)
        titleStyleValue.Size = UDim2.new(1, -titleStyleArW * 2, 1, 0)
        titleStyleValue.BackgroundTransparency = 1
        titleStyleValue.TextColor3 = Color3.fromRGB(220, 220, 220)
        titleStyleValue.Font = Enum.Font.GothamBold
        titleStyleValue.TextSize = 10
        titleStyleValue.ZIndex = 6
        titleStyleValue.TextScaled = true

        local titleStyleNext = Instance.new("TextButton", titleStyleCont)
        titleStyleNext.AnchorPoint = Vector2.new(1, 0)
        titleStyleNext.Position = UDim2.new(1, 0, 0, 0)
        titleStyleNext.Size = UDim2.new(0, titleStyleArW, 1, 0)
        titleStyleNext.BackgroundTransparency = 1
        titleStyleNext.BorderSizePixel = 0
        titleStyleNext.Text = ">"
        titleStyleNext.TextColor3 = Color3.fromRGB(200, 200, 200)
        titleStyleNext.Font = Enum.Font.GothamBold
        titleStyleNext.TextSize = 14
        titleStyleNext.ZIndex = 6

        local function refreshTitleStyleSelector()
            local st = TITLE_STYLES[titleStyleIndex]
            titleStyleValue.Text = st and st.name or "TITLE 1"
        end

        local function setTitleStyleIndex(nextIndex)
            if nextIndex < 1 then
                nextIndex = #TITLE_STYLES
            end
            if nextIndex > #TITLE_STYLES then
                nextIndex = 1
            end

            applyTitleStyle(nextIndex)
            refreshTitleStyleSelector()
        end

        titleStylePrev.MouseButton1Click:Connect(function()
            setTitleStyleIndex(titleStyleIndex - 1)
        end)

        titleStyleNext.MouseButton1Click:Connect(function()
            setTitleStyleIndex(titleStyleIndex + 1)
        end)

        hoverRow(titleStyleRow, ROW_ALPHA)
        refreshTitleStyleSelector()
    end
